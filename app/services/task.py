...
def generate_final_videos(
    ...
    for i in range(params.video_count):
        ...
        final_video_path = utils.build_final_video_path(
            task_id,
            index,
            video_subject=params.video_subject,
            total_count=params.video_count,
        )
...
def start(task_id, params: VideoParams, stop_at: str = "video"):
    utils.refresh_video_output_directory()
    ...
    exported_video_paths = utils.export_final_videos(
        final_video_paths,
        video_subject=params.video_subject,
        task_id=task_id,
    )
    if not exported_video_paths and utils.get_video_output_directory():
        exported_video_paths = [
            path.abspath(video_path)
            for video_path in final_video_paths
            if video_path and path.isfile(video_path)
        ]
    if exported_video_paths:
        logger.success(
            f"exported {len(exported_video_paths)} videos to "
            f"{utils.get_video_output_directory()}"
        )
    ...
    if exported_video_paths:
        kwargs["exported_videos"] = exported_video_paths
    sm.state.update_task(
