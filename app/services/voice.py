...
def _azure_voices_data_file() -> str:
    from app.utils.paths import bundle_root, is_frozen

    if is_frozen():
        return os.path.join(
            bundle_root(), "app", "services", "data", "azure_voices.json"
        )
    return os.path.join(os.path.dirname(__file__), "data", "azure_voices.json")


def _load_azure_voices() -> list[dict]:
    cached = getattr(_load_azure_voices, "_cache", None)
    if cached is None:
        voices_file = _azure_voices_data_file()
        with open(voices_file, "r", encoding="utf-8") as f:
            cached = json.load(f)
        _load_azure_voices._cache = cached
    return cached
